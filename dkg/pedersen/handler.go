...
func (h *Handler) verifiableDecryption(jptr *job) {
	...
	if err != nil {
		...
		h.RLock()
		// ui
		U_i := suite.Point().Mul(h.privShare.V, cp.K) // ui in the paper
		h.RUnlock()
		partial := suite.Point().Sub(cp.C, U_i) //share of this party. needed for decrypting
		s_i := suite.Scalar().Pick(suite.RandomStream())
		UHat := suite.Point().Mul(s_i, cp.K)
		HHat := suite.Point().Mul(s_i, nil)
		hash := sha256.New()
		U_i.MarshalTo(hash)
		UHat.MarshalTo(hash)
		HHat.MarshalTo(hash)
		E_i := suite.Scalar().SetBytes(hash.Sum(nil))
		h.RLock()
		F_i := suite.Scalar().Add(s_i, suite.Scalar().Mul(E_i, h.privShare.V))
		H_i := suite.Point().Mul(h.privShare.V, nil)
		h.RUnlock()

...
func (m minimal) SetCommands(builder node.Builder) {
	...
	sub.SetFlags(
		...
		cli.StringSliceFlag{
			Name:  "threshold",
			Usage: "<hex>, the threshold of the committee",
		},
	)
	...
	sub.SetDescription("encrypt a message and provides a proof. Outputs <hex(K)>:<hex(C)>:<hex(Ubar)>:<hex(E)>:<hex(F)>:<hex(remainder)>")
	sub.SetFlags(
		cli.StringFlag{
			Name:  "message",
			Usage: "the message to encrypt, encoded in hex",
	...
	sub.SetFlags(
		cli.StringFlag{
			Name:  "ciphertexts",
			Usage: "the ciphertext string, as multiple of <hex(K)>:<hex(C)>:<hex(Ubar)>:<hex(E)>:<hex(F)>",
		},
		cli.StringFlag{
			Name:  "GBar",
			Usage: "the second generator",
	...
	sub.SetFlags(
		...
		cli.StringSliceFlag{
			Name:  "thresholdNew",
			Usage: "<Int>, the threshold of the new committee",
		},
	)
